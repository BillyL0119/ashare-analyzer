...
Simulated A-share trading with T+1 rules, realistic commission fees,
and a global leaderboard. Data stored as per-device JSON files.

Commission schedule:
  Buy:  0.03% of trade value (min ¥5)
  Sell: 0.03% + 0.1% stamp duty = 0.13% of trade value (min ¥5)

T+1: shares bought today cannot be sold until tomorrow (Beijing time).
"""
...
_BUY_RATE       = 0.0003   # 0.03%
_SELL_RATE      = 0.0013   # 0.13% (commission + stamp duty)
_MIN_COMMISSION = 5.0
_LOT_SIZE       = 100
...
def _new_account(device_id: str) -> dict:
    ...
    return {
        ...
        "total_value_history": [
            {"date": _beijing_today(), "value": _INITIAL_CASH}
        ],
    }
...
def _compute_portfolio(account: dict) -> dict:
    """Enrich portfolio positions with current price, market value, P&L."""
    ...
    for symbol, pos in account["portfolio"].items():
        ...
        try:
            ...
            price = avg_cost   # fallback to cost if live price unavailable
...
def create_or_get_account(body: DeviceBody):
    ...
    if not history or history[-1]["date"] != today:
        ...
        account["total_value_history"] = history[-90:]  # keep 90 days
        _save_account(account)

    # Compute rank
    all_accs = _all_accounts()
    sorted_accs = sorted(all_accs, key=lambda a: a.get("cash", 0), reverse=True)
    # Rough sort by cash; proper rank needs enriched values
...
def buy_stock(body: BuyBody):
    ...
    # Update portfolio (weighted avg cost)
    ...
    if symbol in portfolio:
        ...
        portfolio[symbol] = {
            "shares":   new_shares,
            "avg_cost": new_avg,
            "buy_date": old["buy_date"],   # keep original date (T+1 from first buy)
        }
    else:
        portfolio[symbol] = {
            "shares":   shares,
...
def _next_trading_day(date_str: str) -> str:
    """Return the next calendar day (simplified; does not skip holidays)."""
